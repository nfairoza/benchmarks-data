...
# Java Path
#JAVA=/home/javauser/oracle-jdk/jdk-17.0.2/bin/java
JAVA=/usr/lib/jvm/zulu-8-amd64/bin/java
...
for ((n=1; $n<=$NUM_OF_RUNS; n=$n+1)); do
  ...
  for ((gnum=1; $gnum<$GROUP_COUNT+1; gnum=$gnum+1)); do
    ...
    for ((jnum=1; $jnum<$TI_JVM_COUNT+1; jnum=$jnum+1)); do
        ...
        echo "    Start $TI_NAME"
        #numactl --cpunodebind=$node --localalloc $JAVA $JAVA_OPTS_TI $SPEC_OPTS_TI -jar /efs/html/SPECjbb2015/specjbb2015.jar -m TXINJECTOR -G=$GROUPID -J=$JVMID $MODE_ARGS_TI > $TI_NAME.log 2>&1 &
    ...
    echo "    Start $BE_NAME"
    #numactl --cpunodebind=$node --localalloc $JAVA $JAVA_OPTS_BE -Xlog:gc*=info:gc-$GROUPID.log $SPEC_OPTS_BE -jar /efs/html/SPECjbb2015/specjbb2015.jar -m BACKEND -G=$GROUPID -J=$JVMID $MODE_ARGS_BE > $BE_NAME.log 2>&1 &
    $JAVA $JAVA_OPTS_BE $SPEC_OPTS_BE -jar /efs/html/SPECjbb2015/specjbb2015.jar -m BACKEND -G=$GROUPID -J=$JVMID $MODE_ARGS_BE > $BE_NAME.log 2>&1 &
    echo -e "\t$BE_NAME PID = $!"
    sleep 1
  done
  ...
  echo "Please monitor $PWD/controller.out for progress"
